...
# Serialises read-modify-write on state.json. Mirrors tools/recipes._recipes_lock.
# Without it, concurrent mutating calls (parallel orchestrator tool_calls,
# multi-user Streamlit, background jobs) race and silently lose updates;
# Windows os.replace also raises PermissionError under contention. Re-entrant
# so mutators can call read_state() while holding the lock without deadlock.
